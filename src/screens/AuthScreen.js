      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.brand}>
            ...
            <Text style={styles.appName}>Swipe<Text style={{ color: COLORS.primary }}>4</Text>Change</Text>
            <Text style={styles.tagline}>Your voice. Your petition. Your impact.</Text>
          </View>
            ...
            <View style={styles.fields}>
              ...
              {mfaResolver ? (
                <View style={styles.mfaBox}>
                  ...
                  <Text style={styles.mfaSub}>Enter the code sent to {mfaHint?.phoneNumber || 'your verified phone'}.</Text>
                  <View style={styles.inputWrap}>
              ...
              <TouchableOpacity onPress={handleSubmit} disabled={loading} activeOpacity={0.9}>
                <LinearGradient colors={[COLORS.tertiary, COLORS.tertiaryContainer]} style={styles.submitBtn}>
                  <Text style={styles.submitText}>{loading ? 'Working...' : mode === 'login' ? 'Sign In' : 'Create Account'}</Text>
                  <MaterialIcons name="arrow-forward" size={18} color={COLORS.onTertiary} />
                </LinearGradient>
              </TouchableOpacity>

              {!firebaseEnabled ? (
                <Text style={styles.devMode}>Local demo mode. Add Firebase env values to enable real authentication.</Text>
              ) : null}
